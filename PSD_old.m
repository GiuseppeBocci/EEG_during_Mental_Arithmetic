...
for c = 1:N_chan
    ...
    for i = 1:band_i
        ...
        % end
        mul = mul + 0.5;
        mat = cell2mat(area_1(:,c,i));
        scatter(mul*ones(1,6), mat,"blue", 'filled');
        outliers_rest = find(mat > y_lim);

        mat = cell2mat(area_2(:,c,i));
        scatter((mul+inc)*ones(1,6), mat,"red", 'filled');
        outliers_task = find(mat > y_lim);

        for k = 1:numel(outliers_rest)
            o_flag = true;
            o = scatter(mul, y_lim-k*2, "black");
            o.DataTipTemplate.DataTipRows(3) = dataTipTextRow('Actual Y', mat(outliers_rest(k),1));
            o.DataTipTemplate.DataTipRows(4) = '';
        end
        for k = 1:numel(outliers_task)
            o_flag = true;
            o = scatter(mul+inc, y_lim-k*2, "black");
            o.DataTipTemplate.DataTipRows(3) = dataTipTextRow('Actual Y', mat(outliers_task(k),1));
            o.DataTipTemplate.DataTipRows(4) = '';
        end
    end
    xticks([1.1  1.6  2.1  2.6]);
    xticklabels({'\theta1', ...
        '\theta2','\beta1', ...
        '\beta2'});
    xlim([0.7, 3]);
